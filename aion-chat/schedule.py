...
ALARM_CMD = _no_backtick(re.compile(r"\[ALARM:(.+?)\|(.+?)\]"))
REMINDER_CMD = _no_backtick(re.compile(r"\[REMINDER:(.+?)\|(.+?)\]"))
MONITOR_CMD = _no_backtick(re.compile(r"\[Monitor:(.+?)\|(.+?)\]"))
SCHEDULE_DEL_CMD = _no_backtick(re.compile(r"\[SCHEDULE_DEL:(.+?)\]"))
SCHEDULE_LIST_CMD = _no_backtick(re.compile(r"\[SCHEDULE_LIST\]"))
THINK_SCHEDULE_CMD = _no_backtick(re.compile(r"\[THINK_SCHEDULE:(\d{1,2}:\d{2})\|(\w+)\|(.+?)\]"))
...
async def _add_schedule(stype: str, trigger_at: str, content: str, origin: str = "aion", origin_room_id: str = "", repeat: str = None):
    sid = f"sch_{int(time.time()*1000)}"
    now = time.time()
    trigger_at = trigger_at.replace("T", " ")
    async with get_db() as db:
        await db.execute(
            "INSERT INTO schedules (id, type, trigger_at, content, created_at, status, origin, origin_room_id, repeat) VALUES (?,?,?,?,?,?,?,?,?)",
            (sid, stype, trigger_at, content, now, "active", origin, origin_room_id, repeat),
        )
        await db.commit()
    await manager.broadcast({"type": "schedule_changed"})
