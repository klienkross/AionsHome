...
### 坑 13：荣耀/华为 TYPE_STEP_COUNTER 后台不投递事件

**现象**：`initStepCounter()` 注册成功（`stepSensor != null`），权限已授予（`ACTIVITY_RECOGNITION`），但 `onSensorChanged` 永远不回调，`latestStepCounter` 始终为 -1。同设备上其他运动 app 能正常获取步数。

**原因**：`SensorManager.registerListener(listener, sensor, SENSOR_DELAY_NORMAL, handler)` 的 4 参数重载不指定 `maxReportLatencyUs`，系统默认可以无限延迟批量投递。荣耀/华为 ROM 在后台服务（即使是前台服务）中利用这一点，直接不投递 `TYPE_STEP_COUNTER` 事件以省电。

**解决**：使用 5 参数重载 `registerListener(listener, sensor, samplingPeriodUs, maxReportLatencyUs, handler)`，显式传 `maxReportLatencyUs = 0` 强制立即投递：
```java
sensorManager.registerListener(stepListener, stepSensor,
        SensorManager.SENSOR_DELAY_NORMAL, 0, mainHandler);
```

**教训**：**在国产 ROM 上使用传感器（尤其是低功耗传感器如 STEP_COUNTER），必须设 `maxReportLatencyUs = 0`，否则系统有权无限延迟投递。即使 `registerListener` 返回 true 也不代表事件会到达。**
